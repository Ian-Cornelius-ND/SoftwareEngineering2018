...
public class Road {
	...
	public void assignGate(CrossingGate gate, CrossingGate changedGate){
		gates.add(gate);
		this.changedGate = changedGate;
		if (carFactory != null)
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, changedGate);  // allows additional gates.  Needs fixing
	}
	
	public void addCarFactory(){
		if (carFactory == null) // We only allow one
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, changedGate);
	}
