...
public class CrossingGate extends Observable implements Observer{
	...
	public void update(Observable o, Object arg) {
		if (o instanceof Train){
			Train train = (Train)o;
			//Create overlap areas for trains approaching/leaving gate
			if(train.getVehicleX() < (anchorX+200) && train.getVehicleX() > (anchorX-200) && train.getVehicleY() == 475){
				currentGateState.approachStation();
			}
			if(train.getVehicleX() < (anchorX-200) && train.getVehicleX() > (anchorX-210) && train.getVehicleY() == 475){
				currentGateState.leaveStation();
			}
			if(train.getVehicleX() > (anchorX+200) && train.getVehicleX() < (anchorX+210) && train.getVehicleY() == 525){
				currentGateState.leaveStation();
			}
			
			if(train.getVehicleX() > (anchorX-200) && train.getVehicleX() < (anchorX+200) && train.getVehicleY() == 525){
				currentGateState.approachStation();
			}
		} 
	}	
